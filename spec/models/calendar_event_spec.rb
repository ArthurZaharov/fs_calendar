...
RSpec.describe CalendarEvent, type: :model do
	...
	describe "get all events for date" do
		...
		it "don't return events" do
			expect(CalendarEvent.get_all_events_for_date(Date.parse("2014-08-06 UTC"))).to match_array []
		end

		it "return yearly and daily events" do
			expect(CalendarEvent.get_all_events_for_date(Date.parse("2015-09-05 UTC"))).to match_array [calendar_event_daily, calendar_event_yearly]
		end

		it "return monthly and daily events" do
			expect(CalendarEvent.get_all_events_for_date(Date.parse("2014-10-06 UTC"))).to match_array [calendar_event_daily, calendar_event_monthly]
		end

		it "return weekly and daily events" do
			expect(CalendarEvent.get_all_events_for_date(Date.parse("2014-09-14 UTC"))).to match_array [calendar_event_daily, calendar_event_weekly]
		end

		it "return once and daily events" do
			expect(CalendarEvent.get_all_events_for_date(Date.parse("2014-09-09 UTC"))).to match_array [calendar_event_once, calendar_event_daily]
		end

		it "return daily event" do
			expect(CalendarEvent.get_all_events_for_date(Date.parse("2014-09-10 UTC"))).to match_array [calendar_event_daily]
		end
	end
	...
	describe "get all events" do
	  ...
	  it "have ASC order" do
	  	expect(CalendarEvent.get_all_events_for_date(Date.parse("2014-09-10 UTC"))).to eq [fresh_calendar_event, newer_calendar_event, older_calendar_event]
	  end
	end
end
